...
public class NearestAttackableTargetGoalForDalekRenegadeImperial<T extends LivingEntity>
  extends TargetGoal
{
  ...
  public NearestAttackableTargetGoalForDalekRenegadeImperial(MobEntity mobIn, Class<T> targetClassIn, boolean checkSight) {
     this(mobIn, targetClassIn, checkSight, false);
  }
  
  public NearestAttackableTargetGoalForDalekRenegadeImperial(MobEntity mobIn, Class<T> targetClassIn, boolean checkSight, boolean nearbyOnlyIn) {
     this(mobIn, targetClassIn, 10, checkSight, nearbyOnlyIn, (Predicate<LivingEntity>)null);
  }
  
  public NearestAttackableTargetGoalForDalekRenegadeImperial(MobEntity mobIn, Class<T> targetClassIn, int targetChanceIn, boolean checkSight, boolean nearbyOnlyIn, @Nullable Predicate<LivingEntity> targetPredicate) {
  ...
  protected void findNearestTarget() {
     ...
     if (this.nearestTarget != null && this.nearestTarget instanceof DalekEntity) {
       DalekEntity dalek = (DalekEntity)this.nearestTarget;
       if (this.field_75299_d instanceof DalekEntity) {
         DalekEntity thisDalek = (DalekEntity)this.field_75299_d;
         if (thisDalek.getDalekData().getType() == DalekType.IMPERIAL || thisDalek.getDalekData().getType() == DalekType.SPECIAL_WEAPONS || thisDalek.getDalekData().getType() == DalekType.RENEGADE) {
           if (thisDalek.getDalekData().getType() == DalekType.IMPERIAL && dalek.getDalekData().getType() != DalekType.RENEGADE) {
             this.nearestTarget = null;
           } else if (thisDalek.getDalekData().getType() == DalekType.SPECIAL_WEAPONS && dalek.getDalekData().getType() != DalekType.RENEGADE) {
             this.nearestTarget = null;
           } else if (thisDalek.getDalekData().getType() == DalekType.RENEGADE && dalek.getDalekData().getType() != DalekType.IMPERIAL && dalek.getDalekData().getType() != DalekType.SPECIAL_WEAPONS) {
             this.nearestTarget = null;
          } 
        } else {
           this.nearestTarget = null;
